package com.github.wucoding.forum.service.notify.service.impl;

import com.github.wucoding.forum.api.model.enums.NotifyStatEnum;
...
import com.github.wucoding.forum.service.notify.repository.dao.NotifyMsgDao;
import com.github.wucoding.forum.service.notify.repository.entity.NotifyMsgDO;
...
public class NotifyMsgListener<T> implements ApplicationListener<NotifyMsgEvent<T>> {
    ...
    private void saveArticleNotify(NotifyMsgEvent<UserFootDO> event) {
        ...
        if (record == null) {
            // 若之前已经有对应的通知，则不重复记录；因为一个用户对一篇帖子，可以重复的点赞、取消点赞，但是最终我们只通知一次
            notifyMsgDao.save(msg);
        }
    }
