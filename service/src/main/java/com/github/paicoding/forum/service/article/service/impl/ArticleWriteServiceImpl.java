...
import com.github.wucoding.forum.api.model.enums.*;
import com.github.wucoding.forum.api.model.event.ArticleMsgEvent;
...
import com.github.wucoding.forum.core.util.id.IdUtil;
...
import com.github.wucoding.forum.service.article.service.ColumnSettingService;
import com.github.wucoding.forum.service.image.service.ImageService;
import com.github.wucoding.forum.service.user.service.AuthorWhiteListService;
...
public class ArticleWriteServiceImpl implements ArticleWriteService {
    ...
    /**
     * 保存帖子，当articleId存在时，表示更新记录； 不存在时，表示插入
     *
     * @param req
     * @return
     */
        ...
        return transactionTemplate.execute(new TransactionCallback<Long>() {
            ...
            public Long doInTransaction(TransactionStatus status) {
                ...
                if (NumUtil.nullOrZero(req.getArticleId())) {
                    ...
                    log.info("帖子发布成功! title={}", req.getTitle());
                } else {
                    articleId = updateArticle(article, content, req.getTagIds());
                    log.info("帖子更新成功！ title={}", article.getTitle());
                }
    ...
    private Long updateArticle(ArticleDO article, String content, Set<Long> tags) {
        // fixme 待补充帖子的历史版本支持：若帖子处于审核状态，则直接更新上一条记录；否则新插入一条记录
    ...
    public void deleteArticle(Long articleId, Long loginUserId) {
        ...
        if (dto != null && !Objects.equals(dto.getUserId(), loginUserId)) {
            // 没有权限
            throw ExceptionUtil.of(StatusEnum.FORBID_ERROR_MIXED, "请确认帖子是否属于您!");
        }
