package com.github.wucoding.forum.core.async;
...
public class AsyncUtil {
    ...
    private static final ThreadFactory THREAD_FACTORY = new ThreadFactory() {
        ...
        public Thread newThread(Runnable r) {
            ...
            thread.setName("wucoding-" + this.threadNumber.getAndIncrement());
            return thread;
        }
    };
    ...
    public static void initExecutorService(int core, int max) {
        ...
        //  1. 小闲屋不存在cpu密集型任务，大部分操作都设计到 redis/mysql 等io操作
        //  2. 统一的异步封装工具，这里的线程池是一个公共的执行仓库，不希望被其他的线程执行影响，因此队列长度为0, 核心线程数满就创建线程执行，超过最大线程，就直接当前线程执行
        //  3. 同样因为属于通用工具类，再加上小闲屋的异步使用的情况实际上并不是非常饱和的，因此空闲线程直接回收掉即可；大部分场景下，cpu * 2的线程数即可满足要求了
