...
import com.github.wucoding.forum.api.model.vo.article.dto.ColumnArticlesDTO;
...
import com.github.wucoding.forum.api.model.vo.seo.Seo;
import com.github.wucoding.forum.api.model.vo.seo.SeoTagVo;
import com.github.wucoding.forum.core.util.DateUtil;
import com.github.wucoding.forum.web.config.GlobalViewConfig;
import com.github.wucoding.forum.web.front.article.vo.ArticleDetailVo;
import com.github.wucoding.forum.web.front.user.vo.UserHomeVo;
...
public class SeoInjectService {
    private static final String KEYWORDS = "小闲屋,开源社区,java,springboot,IT,程序员,开发者,mysql,redis,Java基础,多线程,JVM,虚拟机,数据库,MySQL,Spring,Redis,MyBatis,系统设计,分布式,RPC,高可用,高并发,小闲屋";
    private static final String DES = "小闲屋,一个基于 Spring Boot、MyBatis-Plus、MySQL、Redis、ElasticSearch、MongoDB、Docker、RabbitMQ 等技术栈实现的社区系统，采用主流的互联网技术架构、全新的UI设计、支持一键源码部署，拥有完整的帖子&教程发布/搜索/评论/统计流程等，代码完全开源，没有任何二次封装，是一个非常适合二次开发/实战的现代化社区项目。学编程，就上小闲屋";
    ...
    public void initUserSeo(UserHomeVo user) {
        ...
        String title = "小闲屋 | " + user.getUserHome().getUserName() + " 的主页";
        list.add(new SeoTagVo("og:title", title));
    ...
    public Seo defaultSeo() {
        Seo seo = initBasicSeoTag();
        List<SeoTagVo> list = seo.getOgp();
        list.add(new SeoTagVo("og:title", "小闲屋"));
        ...
        list.add(new SeoTagVo("article:author", "小闲屋"));

        list.add(new SeoTagVo("author", "小闲屋"));
        list.add(new SeoTagVo("title", "小闲屋"));
